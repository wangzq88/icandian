<table id="dg" class="easyui-datagrid" style="height:500px"  url="/index.php?r=region/index"  toolbar="#toolbar" pagination="true"  rownumbers="true" fitColumns="true" singleSelect="true">  
	<thead>  
		<tr>  
        	<th data-options="field:'region_id',hidden:true">ID</th>
...
<div id="toolbar" style="padding:5px 10px;">
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newRegion()">添加区域</a>  
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editRegion()">编辑区域</a>  
<form style="display:inline-block;float:right;" id="search-form">
...
区域：<input type="text" name="region_name" style="width:120px" onkeyup="return searchKeyEventHandler(event);" onkeydown="return searchKeyEventHandler(event);" onkeypress="return searchKeyEventHandler(event);"/>&nbsp;  
...
<form id="ff" method="post" action="/index.php?r=region/create"> 
            <table>  
                <tr>  
                    <td>省份：</td>  
                    <td>
                ...
                <tr>  
                    <td>区域：</td>  
                    <td><input class="easyui-validatebox" type="text" name="region_name" data-options="required:true" /></td>  
                </tr>  
                <tr>  
                    <td>排序：</td>  
...
<form id="edit-ff" method="post" action="/index.php?r=region/update"> 
...
function searchRegion() {
	...
	$("#dg").datagrid('load',{
		...
		region_name:result_list[2].value,
		status:result_list[3].value
	});	
}
...
function editRegion() {
	...
	if (row){	 
		...
		$('#edit-dlg').dialog('open').dialog('setTitle','编辑区域');	
	}
}
