<table id="dg" class="easyui-datagrid" style="height:500px"  url="/index.php?r=sections/index"  toolbar="#toolbar" pagination="true"  rownumbers="true" fitColumns="true" singleSelect="true">  
	<thead>  
		<tr>  
        	<th data-options="field:'section_id',hidden:true">ID</th>
            <th data-options="field:'section_name',width:80,align:'center'">路段</th>         
        	<th data-options="field:'area_id',hidden:true">地段ID</th>
...
<div id="toolbar" style="padding:10px;height:auto">
	<div style="margin-bottom:5px"> 
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newSection()">添加路段</a>  
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editSection()">编辑路段</a>  
    </div>
    <div>
<form style="display:inline-block;" id="search-form">
        ...
        地段：<select name="shop_area" id="shop_area" onkeyup="return searchKeyEventHandler(event);" onkeydown="return searchKeyEventHandler(event);" onkeypress="return searchKeyEventHandler(event);">
        </select>&nbsp;             
		路段：<input type="text" name="section_name" style="width:120px" onkeyup="return searchKeyEventHandler(event);" onkeydown="return searchKeyEventHandler(event);" onkeypress="return searchKeyEventHandler(event);"/>&nbsp;  
...
<div id="dlg" class="easyui-dialog" style="width:500px;height:350px;padding:10px 20px 0 50px"
    buttons="#dlg-buttons" data-options="resizable:false,modal:true,closed:true">
        <div style="text-align:center;display:none;font-weight:bold;" id="create_tip">路段已经成功添加！</div>
<form id="ff" method="post" action="/index.php?r=sections/create"> 
            <table>  
                <tr>  
                    <td>省份：</td>  
                    <td>
                ...
                <tr>  
                    <td>地段：</td>  
                    <td><select name="area_id" id="ff_area">
                    </select>
                    </td>  
                </tr>                 
                <tr>  
                    <td>路段：</td>  
                    <td><input class="easyui-validatebox" type="text" name="section_name" data-options="required:true" /></td>  
                </tr>                   
                <tr>  
                    <td>排序：</td>  
...
<form id="edit-ff" method="post" action="/index.php?r=sections/update"> 
	<input type="hidden" name="section_id" />
            <table>  
                ...
                <tr>  
                    <td>地段：</td>  
                    <td>
                    	<select name="area_id" id="edit_ff_area">
        				</select>	
        			</td>  
                </tr>                                                  
                <tr>  
...
//搜索
 ...
 ?>
  var areas = new Array(<?php echo count($areas);?>);
 <?php 
 	if($areas && is_array($areas)) {
		foreach($areas as $key => $area) {
			echo 'areas['.$key.'] = {"area_id":"'.$area['area_id'].'","area_name":"'.$area['area_name'].'","region_id":"'.$area['region_id'].'"};';
			echo "\n";
		}
	}
 ?> 
...
$('#shop_region').click(function(event) {
	 $('#shop_area').empty();
	 var regionID = $(this).val();
	 var i = 0;
	 $('#shop_area').append('<option value="0">不限</option>');
	 for(i in areas) {
...
function searchRegion() {
	...
	$("#dg").datagrid('load',{
		...
		shop_area: result_list[3].value,
		section_name:result_list[4].value,
		status:result_list[5].value
	});	
}
...
$('#ff_city').click(function(event) {
	 ...
	 $('#ff_region').trigger('click');
});
$('#ff_region').click(function(event) {
	 $('#ff_area').empty();
	 var regionID = $(this).val();
	 var i = 0;
	 for(i in areas) {
		 if(areas[i].region_id == regionID) {
			 $('#ff_area').append('<option value="'+areas[i].area_id+'">'+areas[i].area_name+'</option>');
		 }
	 }
	 $('#ff_area').trigger('click');
});
//$('#dlg').dialog({  
//    onClose:function(){  
//        $('#dg').datagrid('reload');  
...
$('#edit_ff_region').click(function(event) {
	 $('#edit_ff_area').empty();
	 var regionID = $(this).val();
	 var i = 0;
	 for(i in areas) {
		 if(areas[i].region_id == regionID) {
			 $('#edit_ff_area').append('<option value="'+areas[i].area_id+'">'+areas[i].area_name+'</option>');
		 }
	 }
});
function editSection() {
	...
	if (row){	 
		...
		$('#edit_ff_region').trigger('click');
		$('#edit_ff_area').val(row.area_id);
		$('#edit-dlg').dialog('open').dialog('setTitle','编辑路段');	
	}
}
