...
<table id="dg" style="height:500px" data-options="url:'/index.php?r=adminFeedback/index',toolbar:'#toolbar',pagination:true,rownumbers:true,fitColumns:true,singleSelect:false,pageList:[10,15]">  
	<thead>  
		<tr>  
            ...
            <th data-options="field:'content',align:'center'" width="30%">反馈信息</th>
            <th data-options="field:'flag',hidden:true">类型</th>
            <th data-options="field:'flag_text',align:'center'" width="10%">类型</th>
...
<div style="margin-bottom:5px"> 
        ...
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteFeedback()">删除</a> 
        </div>
         <div> 
<form style="display:inline-block;padding-left:5px;" action="/index.php?r=adminFeedback/already" id="search-form">
        用户名：<input type="text" name="username" style="width:120px" onkeyup="return enterKeyEventHandler(event,searchFeedback);" />&nbsp;
 		反馈类型：<select name="flag" onkeyup="return enterKeyEventHandler(event,searchFeedback);" >
        	<option value="0">——</option>
        	<option value="1">催单</option>
        	<option value="2">网站错误</option>
            <option value="3">功能建议</option>
            <option value="4">投诉</option>
        </select>&nbsp;      
        显示：<select name="status" onkeyup="return enterKeyEventHandler(event,searchFeedback);" >
...
<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-search" onclick="searchFeedback()">搜索</a> 
</form>        
</div>
<div class="clear"></div>
</div>
...
$('#dg').datagrid({  
  ...
  detailFormatter: function(rowIndex, rowData){  		
	  ...
	  '<div class="c-item"><span class="c-label">反馈信息:</span>  <span class="c-content">'+rowData.content+'</span></div>'+
	  '<div class="c-item"><span class="c-label">反馈类型:</span> <span class="c-content"> '+rowData.flag_text+'</span></div>'+
...
function searchFeedback() {
	...
	$("#dg").datagrid('load',{
		...
		flag:result_list[1].value,
		status:result_list[2].value,
		"add_time[0]":result_list[3].value,
		"add_time[1]":result_list[4].value		
	});	
}
	...
	if (rows){
		$.messager.confirm('确认','一旦删除将不能恢复，你确定要删除选中的记录吗？',function(r){
			if (r){
				...
				ids = ids.join(',');		
				$.post('/index.php?r=adminFeedback/delete',{id:ids},function(result){
...
function setStatusFlag(status) {
	...
	if (rows){
		...
		ids = ids.join(',');
		$.post('/index.php?r=adminFeedback/update',{id:ids,status:status},function(result){
			$('#dg').datagrid('reload'); 	// reload the user data
		},'json');
	}			
}
